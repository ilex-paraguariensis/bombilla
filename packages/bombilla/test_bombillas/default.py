from data.loaders.cifar10.data_loader import CifarLightningDataModule

data = CifarLightningDataModule(
    location="./data/cifar10", batch_size=32, image_size=[256, 256], crop_size=4
)


from models.resnet.resnet import BasicBlock

basic_block_block = BasicBlock()


from torch.nn import BatchNorm2d

batch_norm2d_norm_layer = BatchNorm2d()


from pytorch_lightning.callbacks import EarlyStopping

early_stopping_0 = EarlyStopping(monitor="val_loss", patience=10, mode="min")


from aim.pytorch_lightning import AimLogger

aim_logger_logger = AimLogger(
    experiment="default", train_metric_prefix="train_", val_metric_prefix="val_"
)


from pytorch_lightning import Trainer

trainer = Trainer(
    gpus=1,
    max_epochs=100,
    precision=16,
    gradient_clip_val=0.5,
    enable_checkpointing=True,
    callbacks=[early_stopping_0, model_checkpoint_1],
    logger=aim_logger_logger,
)


from models.resnet.resnet import ResNet

classifier = ResNet(
    block=basic_block_block,
    layers=[3, 4, 6, 3],
    num_classes=10,
    in_channels=3,
    zero_init_residual=False,
    groups=1,
    width_per_group=64,
    replace_stride_with_dilation=[False, False, False],
    norm_layer=batch_norm2d_norm_layer,
)


from trainers.base_classification.base_classification import (
    LightningClassificationModule,
)

pl_model = LightningClassificationModule(
    classifier=classifier,
    optimizers={
        "optimizer": optimizer,
        "lr_scheduler": {
            "monitor": "val_loss",
            "scheduler": reduce_l_r_on_plateau_scheduler,
        },
    },
)


from torch.optim import Adam

optimizer = Adam(lr=0.0004, betas=[0.5, 0.999], params=classifier.parameters())


from torch.optim.lr_scheduler import ReduceLROnPlateau

reduce_l_r_on_plateau_scheduler = ReduceLROnPlateau(
    optimizer=optimizer,
    mode="min",
    factor=0.5,
    threshold=1e-8,
    threshold_mode="rel",
    patience=0,
    verbose=True,
)


returns = {
    "train": [trainer.fit(model=pl_model, datamodule=data)],
    "test": [trainer.test(model=pl_model, datamodule=data)],
}
